...
def save_asset(asset_id):    
    ...
    dumps_path = pathlib.Path(construct_file_name(asset_id, ASSET_TYPE.DUMP))
    if not dumps_path.exists() and not any(dumps_path.glob("*.dump.json")):
        output_data = {}
        ...
        for key, value in tables.items():
            if key != "Data Product contains Table":
                if key not in output_data:
                    output_data[key] = []
                for i in range(len(value)):    
                    output_data[key].append(value[i][1])
        
        if "Data Product contains Table" in tables:
            for key, value in tables.items():
                dump_data = output_data
                for i in range(len(value)):    
                    dump_data[key] = [value[i][1]]    
                    dump_json(asset_id, ASSET_TYPE.DUMP, dump_data, uuid.uuid4())
    else: 
        print(f"Skipped asset processing of dump for asset: {asset_id}")
    

    for dump_file in dumps_path.glob("*.dump.json"):
        if dump_file.is_file():
            render_template(
                asset_id=asset_id,
                data_path=dump_file,
                template_path=construct_file_name(ASSET_TYPE.CONTRACT_TEMPLATE)
            )
