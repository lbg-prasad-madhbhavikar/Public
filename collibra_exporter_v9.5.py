...
def extract_collibra_data(
    ...
    if parallelism_count <= 1:
        ...
        # Can we have a checkpointing mechanism here as well? is it required?
...
def construct_file_name(asset_id=None, type:ASSET_TYPE=ASSET_TYPE.ROOT, file_name=""):
    ...
    match type:
        ...
        case ASSET_TYPE.ASSET_RELATIONS_SOURCE_QUEUE:
            file_name = f"{asset_id}/ASSET_RELATIONS_SOURCE_QUEUE/{file_name}.relation.json"
        case ASSET_TYPE.TABLE:
            file_name = f"{asset_id}/TABLES/{file_name}.table.json"
        case ASSET_TYPE.COLUMNS:
            file_name = f"{asset_id}/COLUMNS/{file_name}.column.json"
        case ASSET_TYPE.CONTRACT:
            file_name = f"contracts/{asset_id}/{file_name}.contract.json"
        case ASSET_TYPE.DUMP:
            if file_name == "":
                file_name = f"{asset_id}/dumps"
            else:
                file_name = f"{asset_id}/dumps/{file_name}.dump.json"
        ...
        case ASSET_TYPE.CONTRACT_TEMPLATE:
            return "contract.template.v10.json"
        case ASSET_TYPE.ERROR:
            file_name = f"{asset_id}/error.log"
        case _:
            file_name = f"{asset_id}/{file_name}.json"
        ...
        case ASSET_TYPE.ASSET_RELATIONS_SOURCE_QUEUE:
            for item in data:
                ...
                with open(item_file_path, "w", encoding="utf-8") as f:
                    f.write(orjson.dumps(item, option=orjson.OPT_INDENT_2).decode())
...
def render_template(asset_id, data_path, template_path):
    ...
    contract_file = construct_file_name(asset_id, ASSET_TYPE.CONTRACT, asset_id)
    pathlib.Path(contract_file).parent.mkdir(parents=True, exist_ok=True)
...
def check_and_invoke(asset_id, type:ASSET_TYPE, fn, id, file_name=""):
    ...
    if not pathlib.Path(file_name).exists():
        ...
        result = orjson.loads(open(file_name, 'r', encoding='utf-8').read())
...
def collect_and_cache_results(asset_id, type:ASSET_TYPE, file_path, future, target_file_name=""):
    if future:
        ...
        if ASSET_TYPE.ASSET_RELATIONS_SOURCE_QUEUE == type or ASSET_TYPE.TABLE == type and ASSET_TYPE.COLUMNS == type:
            ...
            result = orjson.loads(open(file_path, 'r', encoding='utf-8').read())
...
def process_relational_data(asset_id, asset_relations_source_queue, limit=10, parrallelism_count=8):
...
def save_asset(asset_id):    
    ...
    if not pathlib.Path(construct_file_name(asset_id, ASSET_TYPE.DUMP, "")).exists():
        ...
        with open(asset_details_file_name, 'r', encoding='utf-8') as f:
            asset_details = orjson.loads(f.read())
                ...
                for table_folder in pathlib.Path(column_table_folder).iterdir():
                    ...
                    for column_file in folder_dir.iterdir():
                        ...
                        with open(column_file, 'r', encoding='utf-8') as f:
                            column_data = orjson.loads(f.read())
                            table_relation_id = table_folder.name.split(os.sep)[-1]
                            # this code needs refactoring and optimization for DS
                            for table_entry in table_data:
                                if table_entry[1]["id"] == table_relation_id:
                                    table_entry[1]["columns"].append(column_data)

        for key, value in tables.items():
            if key not in output_data:
                output_data[key] = []
            for i in range(len(value)):    
                output_data[key].append(value[i][1])
        
        dump_json(asset_id, ASSET_TYPE.DUMP, output_data, asset_id)
    else: 
        print(f"Skipped asset processing for dump for asset: {asset_id}")
    
    render_template(
        asset_id=asset_id,
        data_path=construct_file_name(asset_id, ASSET_TYPE.DUMP, asset_id),
        template_path='contract.template.v10.json'
    )
...
if __name__ == "__main__":
    ...
    if(action == "scrape"):
        ...
        with open('scraping_targets_config.json', 'r', encoding='utf-8') as f:
            scraping_targets = orjson.loads(f.read())
        ...
        for target in targets:
            ...
            try:
                ...
                with open(construct_file_name(asset_id, ASSET_TYPE.ERROR), "w", encoding="utf-8") as error_log:
        ...
        for item_file in assets_dir.iterdir():
            if item_file.is_dir() and item_file.name != "contracts":
                asset_id = item_file.name
                error_log_path = construct_file_name(asset_id, ASSET_TYPE.ERROR)
                if pathlib.Path(error_log_path).exists():
                    print(f"Skipping asset [{asset_id}] due to previous errors during data extraction.")
                    continue
                save_asset(asset_id)
